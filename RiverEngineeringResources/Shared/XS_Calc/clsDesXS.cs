namespace RiverEngineeringResources.Shared.XS_Calc
{
    ...
    public class clsDesXS
    {
        ...
        // Public GlobalStageStationList As New List(Of String)
        ...
        public clsDesXS()
        {
            // Specify the path to the file in the wwwroot folder
            string filePath = Path.Combine("wwwroot", "sample-data", "2023_XSDesignerV1m.xlsm");

            // Use System.IO.File to read the file content
            byte[] fileContent = File.ReadAllBytes(filePath);

            // Open the workbook from the file content
            ...
            // GlobalStageStationList = New List(Of String)
        ...
        public void updateDesXSStageStation()
        {
            m_Left_3rd = "";
            m_Left_2nd = "";
            m_Left_Bank = "";
            ...
            foreach (DataRow dr in xsDataTable.Rows) // set the values from the datatable to the public properties for the property grid
            {
                switch (dr["Stage"].ToString())
                {
                    case "Left 3rd Stage":
                        {
                            this.m_Left_3rd = dr["Station"].ToString();
                            break;
                        }

                    case "Left 2nd Stage":
                        {
                            this.m_Left_2nd = dr["Station"].ToString();
                            break;
                        }

                    case "Left Bankfull":
                        {
                            this.m_Left_Bank = dr["Station"].ToString();
                            break;
                        }

                    case "Left Inner Berm":
                        {
                            this.m_Left_IB = dr["Station"].ToString();
                            break;
                        }

                    case "Thalweg":
                        {
                            this.m_Thalweg = dr["Station"].ToString();
                            break;
                        }

                    case "Right Inner Berm":
                        {
                            this.m_Right_IB = dr["Station"].ToString();
                            break;
                        }

                    case "Right Bankfull":
                        {
                            this.m_Right_Bank = dr["Station"].ToString();
                            break;
                        }

                    case "Right 2nd Stage":
                        {
                            this.m_Right_2nd = dr["Station"].ToString();
                            break;
                        }

                    case "Right 3rd Stage":
                        {
                            this.m_Right_3rd = dr["Station"].ToString();
                            break;
                        }
                }
            }
        ...
        public void Calculate_Full() // full updates the XS DATA
        {
            ...
            // ****Clears data... may or may not be desired
            // xsDataSet.Clear()
            // ****Clears data and all table schema.. may or may not be desired
            // xsDataSet.Tables.Clear()
            ...
            // lock the workbook before editing it
            ...
            try
            {
                // Do some work...
                // MsgBox("locked")
                // dataSet = WorkbookView1.ActiveWorkbook.GetDataSet("Threshold", SpreadsheetGear.Data.GetDataFlags.None)
                ...
                int numRow = xsDataTable.Rows.Count;

                DataTable dummyDTnValue = new DataTable();
                dummyDTnValue.TableName = "n_values";
                dummyDTnValue.Columns.Add("n Value", typeof(string));

                dummyDTnValue.Columns["n Value"].DataType = typeof(string); // pull just the "n" value data

                foreach (DataRow row in xsDataTable.Rows)
                {
                    //if (!IsDBNull(row["n Value"]))
                    //    dummyDTnValue.ImportRow(row);
                    //else
                    //    dummyDTnValue.Rows.Add("");

                    if (!Convert.IsDBNull(row["n Value"]))
                        dummyDTnValue.ImportRow(row);
                    else
                    {
                        DataRow newRow = dummyDTnValue.NewRow();
                        newRow["n Value"] = ""; // Assuming "n Value" is the relevant column name
                        dummyDTnValue.Rows.Add(newRow);
                    }
                }


                // add blank rows for the rest of the rows
                // For i As Integer = dummyDTnValue.Rows.Count - 1 To 19 - 1 Step 1
                // Dim blankRow As DataRow
                // blankRow = dummyDTnValue.NewRow

                // blankRow("n Value") = ""  ' work with just the "n" value data
                // dummyDTnValue.Rows.Add(blankRow)

                // Next

                myXSWorkbook.Worksheets["Data"].Range["C2:C20"].CopyFromDataTable(dummyDTnValue, SpreadsheetGear.Data.SetDataFlags.NoColumnHeaders);


                myXSWorkbook.Worksheets["Data"].Range["F3"].Value = LeftLimit;
                myXSWorkbook.Worksheets["Data"].Range["G3"].Value = RightLimit;
                myXSWorkbook.Worksheets["Data"].Range["F7"].Value = Slope;
                myXSWorkbook.Worksheets["Data"].Range["F10"].Value = WSE;

                myXSWorkbook.Worksheets["Data"].Range["I3"].Value = Left_3rd;
                myXSWorkbook.Worksheets["Data"].Range["I4"].Value = Left_2nd;
                myXSWorkbook.Worksheets["Data"].Range["I5"].Value = Left_Bank;
                myXSWorkbook.Worksheets["Data"].Range["I6"].Value = Left_IB;
                myXSWorkbook.Worksheets["Data"].Range["I7"].Value = Thalweg;
                myXSWorkbook.Worksheets["Data"].Range["I8"].Value = Right_IB;
                myXSWorkbook.Worksheets["Data"].Range["I9"].Value = Right_Bank;
                myXSWorkbook.Worksheets["Data"].Range["I10"].Value = Right_2nd;
                myXSWorkbook.Worksheets["Data"].Range["I11"].Value = Right_3rd;

                myXSWorkbook.Worksheets["Data"].Range["J14"].Value = ShearMethod;

                myXSWorkbook.Worksheets["Data"].Range["J22"].Value = ShieldsValue;
                myXSWorkbook.Worksheets["Data"].Range["J19"].Value = myAndrewsProperties.D50_Bed;
                myXSWorkbook.Worksheets["Data"].Range["K19"].Value = myAndrewsProperties.D50_Bar;
                myXSWorkbook.Worksheets["Data"].Range["L19"].Value = myAndrewsProperties.Largest_D;

                myXSWorkbook.Worksheets["Data"].Range["N4"].Value = numStages;
                myXSWorkbook.Worksheets["Data"].Range["O4"].Value = descXS;
                myXSWorkbook.Worksheets["Data"].Range["P4"].Value = isInnerBerm;

                myXSWorkbook.Worksheets["Data"].Range["P6"].Value = myInnerBermProperties.WIB;
                myXSWorkbook.Worksheets["Data"].Range["P7"].Value = myInnerBermProperties.DmaxIB;

                myXSWorkbook.Worksheets["Data"].Range["P9"].Value = myStage1Properties.Width;
                myXSWorkbook.Worksheets["Data"].Range["P10"].Value = myStage1Properties.Depth;
                myXSWorkbook.Worksheets["Data"].Range["P11"].Value = myStage1Properties.mbanks;
                myXSWorkbook.Worksheets["Data"].Range["P12"].Value = myStage1Properties.ybed;
                myXSWorkbook.Worksheets["Data"].Range["P13"].Value = myStage1Properties.thalweg_shift;
                myXSWorkbook.Worksheets["Data"].Range["P14"].Value = myStage1Properties.Mtieout_L;
                myXSWorkbook.Worksheets["Data"].Range["P15"].Value = myStage1Properties.Mtieout_R;
                myXSWorkbook.Worksheets["Data"].Range["P16"].Value = myStage1Properties.Roundness;
                myXSWorkbook.Worksheets["Data"].Range["P18"].Value = myStage2Properties.D2stg;
                myXSWorkbook.Worksheets["Data"].Range["P19"].Value = myStage2Properties.Mbanks_2stg;
                myXSWorkbook.Worksheets["Data"].Range["P20"].Value = myStage2Properties.Wbench_L;
                myXSWorkbook.Worksheets["Data"].Range["P21"].Value = myStage2Properties.Wbench_R;
                myXSWorkbook.Worksheets["Data"].Range["P22"].Value = myStage2Properties.Ybench_L;
                myXSWorkbook.Worksheets["Data"].Range["P23"].Value = myStage2Properties.Ybench_R;
                myXSWorkbook.Worksheets["Data"].Range["P24"].Value = myStage2Properties.Mtieout_2stg_L;
                myXSWorkbook.Worksheets["Data"].Range["P25"].Value = myStage2Properties.Mtieout_2stg_R;
                myXSWorkbook.Worksheets["Data"].Range["P27"].Value = myStage3Properties.D3stg;
                myXSWorkbook.Worksheets["Data"].Range["P28"].Value = myStage3Properties.Mbanks_3stg;
                myXSWorkbook.Worksheets["Data"].Range["P29"].Value = myStage3Properties.W3rdstage_L;
                myXSWorkbook.Worksheets["Data"].Range["P30"].Value = myStage3Properties.W3rdstage_R;
                myXSWorkbook.Worksheets["Data"].Range["P31"].Value = myStage3Properties.y3rdStage_L;
                myXSWorkbook.Worksheets["Data"].Range["P32"].Value = myStage3Properties.y3rdStage_R;
                myXSWorkbook.Worksheets["Data"].Range["P33"].Value = myStage3Properties.Mtieout_3stg_L;
                myXSWorkbook.Worksheets["Data"].Range["P34"].Value = myStage3Properties.Mtieout_3stg_R;
                myXSWorkbook.Worksheets["Data"].Range["P37"].Value = myAdvancedXSProperties.Left_BKF_Height_Multiplier;
                myXSWorkbook.Worksheets["Data"].Range["P38"].Value = myAdvancedXSProperties.Right_BKF_Height_Multiplier;
                myXSWorkbook.Worksheets["Data"].Range["P39"].Value = myAdvancedXSProperties.Left_Mbanks_BKF_Multiplier;
                myXSWorkbook.Worksheets["Data"].Range["P40"].Value = myAdvancedXSProperties.Right_Mbanks_BKF_Multiplier;
                myXSWorkbook.Worksheets["Data"].Range["P41"].Value = myAdvancedXSProperties.Left_BKF_Bottom_Slope_Multiplier;
                myXSWorkbook.Worksheets["Data"].Range["P42"].Value = myAdvancedXSProperties.Right_BKF_Bottom_Slope_Multiplier;
                myXSWorkbook.Worksheets["Data"].Range["P43"].Value = myAdvancedXSProperties.Y_datum;
                myXSWorkbook.Worksheets["Data"].Range["P44"].Value = myAdvancedXSProperties.X_datum;
                ...
                foreach (string val in hydraulics_dataArray)
                    ChangeFieldType(xsDataSet, "hydraulics_data", val, typeof(double));
                //ChangeFieldType(xsDataSet, "hydraulics_data", val, doub.GetType());// CONVERT STRINGS TO DOUBLES AND SET NULLS TO ""


                // Console.Write("*****************1*************************")
                // Console.Write(DumpDataTable(dataSet.Tables("xs_data")))
                ChangeFieldType(xsDataSet, "xs_data", "Station", typeof(double)); // CONVERT STRINGS TO DOUBLES AND GET RID OF NULLS
                ChangeFieldType(xsDataSet, "xs_data", "Elevation", typeof(double)); // CONVERT STRINGS TO DOUBLES AND GET RID OF NULLS
                ...
                foreach (DataRow dr in xsDataSet.Tables["xs_data"].Rows)
                {
                    ...
                    xsDataTable.Rows[rowIndex]["Station"] = dr["Station"];
                    xsDataTable.Rows[rowIndex]["Elevation"] = dr["Elevation"];
                    xsDataTable.Rows[rowIndex]["Stage"] = dr["Stage"];
                    //xsDataTable.Rows.Item(xsDataSet.Tables("xs_data").Rows.IndexOf(dr)).Item("Station") = dr("Station");
                    //xsDataTable.Rows.Item(xsDataSet.Tables("xs_data").Rows.IndexOf(dr)).Item("Elevation") = dr("Elevation");
            ...
            // force the update that's calls binding complete method
            // xsDataTable.AcceptChanges()


            // Console.Write(DumpDataTable(xsdatatable))
            // Console.WriteLine("Ready to plot")
            ...
            // update the stage-station list in property grid
            updateDesXSStageStation();
        }
        ...
        //    // xsChart.ChartAreas("ChartArea1").AxisY2.MinorGrid = xsChart.ChartAreas("ChartArea1").AxisY.MinorGrid
        ...
        public void ChangeIsBrowsableTo(bool @bool, string PD)
        {
            ...
            oDescriptor = TypeDescriptor.GetProperties(typeof(clsDesXS))["test"];
        ...
        public void CalculatePoints()
        {
            ...
            else if (isInnerBerm == "No")
            {
                ...
                // RIGHT SIDE
                R1x = 0.001 + CLx;
                R2x = ((0.25 + (0.25 * SubcategoryStage1.Roundness)) * (R1x + CLx + (0.5 * Wbed))) + R1x;
    ...
    public class clsStageOptions : System.ComponentModel.StringConverter
    {

        // Public theList As List(Of String)
        // Dim stageStationList As ICollection
        // Public Sub updateStageStationValues(passedList As ICollection)
        // stageStationList = passedList
        // MsgBox("created")
        // End Sub

        public override TypeConverter.StandardValuesCollection GetStandardValues(ITypeDescriptorContext context)
        {
            return new TypeConverter.StandardValuesCollection(globalVars.GlobalStageStationList);
        }
